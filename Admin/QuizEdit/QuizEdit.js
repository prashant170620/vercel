...
async function loadQuiz(quizId) {
    ...
    let res = await fetch(`http://localhost:3000/admin/getQuiz?quizId=${quizId}`);
...
async function updateMarks(ele) {
    let marks = ele.value;
    let res = await fetch(`http://localhost:3000/admin/addmarks?quizId=${Quiz.id}&marks=${marks}`);
...
async function updateTitle(ele) {
    let title = ele.value;
    let res = await fetch(`http://localhost:3000/admin/updatequiz?quizId=${Quiz.id}&newtitle=${title}`);
    let { response } = await res.json();
    if (!response) {
...
async function updateQuestion(ele) {
    ...
    if(ele.id){
        ...
        let res = await fetch(`http://localhost:3000/admin/updatequestion?questionId=${ele.id}&des=${des}`);
        let { response } = await res.json();
        if (response) {
...
async function insertQuestion(ele){
    let res = await fetch(`http://localhost:3000/admin/addquestion?quizId=${Quiz.id}&questiondes=${ele.value}`);
...
async function updateOption(ele){
    ...
    if(ele.id !== "undefined" && ele.id !== ''){
         let res = await fetch(`http://localhost:3000/admin/updateoption?optionId=${ele.id}&des=${ele.value}`);
         let {response} = await res.json();
         if(response){
            let flag = false;
...
async function insertOption(ele){
    ...
    if(questionId){
        let res = await fetch(`http://localhost:3000/admin/addoption?questionId=${questionId}&optiondes=${ele.value}`);
...
async function addRightOption(ele){
    ...
    if(optionId){
        let questionId = ele.parentElement.parentElement.parentElement.firstElementChild.firstElementChild.id;
        let res = await fetch(`http://localhost:3000/admin/addrightoption?questionId=${questionId}&rightoptionId=${ele.id}`);
        let {response} = await res.json();
        if(response){
